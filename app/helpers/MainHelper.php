...
class mainHelper{
	...
	*/
	...
	/**
	* Função estática - Retorna valor de array c/ campos únicos
	* @param array $array - array c/ valores da tabela
	* @param string $value - Valor do option do select
	* @param string $descri - Texto do option
	* @return array $newArray - Array c/ valores atribuidos
	* @author Rafael Domingues Teixeira
	*/
	public static function getSubValue($data) {
		foreach ($data as $key => $value) {
			if(is_array($value))
				$value = Self::getSubValue($value);
		}
		return $value;
	}
	...
	*/
	...
	// 	return $pdf;
	// }
	public static function exportPdf($content, $tipo){
		return View::make('pdf.'.$tipo, compact('content'));
	}
